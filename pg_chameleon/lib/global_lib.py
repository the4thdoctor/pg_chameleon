...
import traceback 
from .notifiers import rollbar_notifier, email_notifier


class notifier(object):
    def __init__(self, args, wholeconf, logger):
        kind = wholeconf["notifier"]["kind"]
        if kind == "rollbar":
            self.notifier = rollbar_notifier(wholeconf["notifier"]["attributes"]["rollbar_key"],
                                             wholeconf["notifier"]["attributes"]["rollbar_env"], args.notify_level, logger)
        elif kind == "email":
            self.notifier = email_notifier(wholeconf["notifier"]["attributes"], args.notify_level, logger)
        else:
            raise ValueError("Unknown type of notifier: %s" % kind)

    def send_message(self, message, level):
        self.notifier.send_message(message, level)
...
class replica_engine(object):
    ...
    def __init__(self, args):
        ...
        # self.notifier = rollbar_notifier(self.config["rollbar_key"],self.config["rollbar_env"] , self.args.notify_level ,  self.logger )
        self.notifier = notifier(self.args, self.config, self.logger)
