...
(
	...
	(
		SELECT
			array_agg(distinct agg.i_id_event ) as i_id_event,
			agg.v_table_name,
			agg.v_schema_name,
				...
				(
					...
					(
						SELECT 
							...
							coalesce(log.jsb_event_after,'{"foo":"bar"}'::jsonb) as jsb_event_after,--||format('{"i_id_event":%s}',log.i_id_event)::jsonb as jsb_event_after,
							log.jsb_event_before,
							trim(both ' ' from log.t_query) as t_query,
